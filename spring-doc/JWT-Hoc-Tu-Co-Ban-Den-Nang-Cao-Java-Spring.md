...
public class CartController {
    ...
    @PostMapping("/add")
    public ResponseEntity<Map<String, Boolean>> addToCart(
            @RequestBody CartRequest request,
            @RequestAttribute("userId") String userId,
            HttpServletResponse response) {
        
        List<CartItem> cart = request.getCart();
    ...
    public ResponseEntity<Map<String, Object>> getCart(
            @CookieValue(value = "cart", required = false) String token) {
        
        if (token == null || token.isEmpty()) {
            return ResponseEntity.ok(Map.of("cart", Collections.emptyList()));
        }
        ...
        try {
            ...
            List<Map<String, Object>> cart = 
                (List<Map<String, Object>>) claims.get("cart");
            
            return ResponseEntity.ok(Map.of("cart", 
                cart != null ? cart : Collections.emptyList()));
...
try {
    ...
    // Có thể bị tấn công algorithm confusion!
}

// ✅ ĐÚNG: Luôn kiểm tra algorithm
try {
    Claims claims = Jwts.parser()
        .verifyWith(secretKey)
        .require("alg", "HS256") // Chỉ chấp nhận HS256
        .build()
        .parseSignedClaims(token)
        .getPayload();
...
public ResponseEntity<LoginResponse> login(@RequestBody LoginRequest request) {
    ...
    String refreshToken = Jwts.builder()
        ...
        .issuedAt(new Date())
        .expiration(new Date(System.currentTimeMillis() + 7 * 24 * 60 * 60 * 1000)) // 7 ngày
...
private SecretKey getAccessSecretKey() {
    String secret = environment.getProperty("jwt.access.secret");
    return Keys.hmacShaKeyFor(secret.getBytes());
}

private SecretKey getRefreshSecretKey() {
    String secret = environment.getProperty("jwt.refresh.secret");
    return Keys.hmacShaKeyFor(secret.getBytes());
}
...
System.out.println("Encrypted JWT: " + encrypted);

// Giải mã với private key
JWEObject jweObject2 = JWEObject.parse(encrypted);
jweObject2.decrypt(new RSADecrypter(rsaKey));

String decrypted = jweObject2.getPayload().toString();
